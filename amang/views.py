...
from .models import Attendance, AttendanceStatus, Information, Member, Schedule
...
def timetable(request: HttpRequest):
    ...
    return render(
        ...
        {"members": members, "user": request.user},
    )
...
def update_schedule(request: HttpRequest):
    ...
    schedule.start = request.POST["start"]  # type: ignore
    schedule.end = request.POST["end"]  # type: ignore
    if "title" in request.POST:
        schedule.title = request.POST["title"]
    if "director" in request.POST:
        schedule.director = request.POST["director"]
    schedule.save()
...
def advanced(request: HttpRequest, method: str = "settings"):
    ...
    if not request.user.is_authenticated or not request.user.is_superuser:  # type: ignore
        return index(request)
            ...
            for member in Member.objects.all():
                if f"pk[{member.pk}]" in request.POST:
                    att = int(
                        ...
                        if (
                            value := str(
                                request.POST[f"attendance[{member.pk}]"]
                            )
                        )
                        != ""
                        else 0
                    ...
                    attendance = Attendance(
                        member=member,
                        date=date,
                        value=AttendanceStatus.by_index(att),
                    )
                    attendance.save()
